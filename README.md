...
PokéAPI [requires caching as a condition of use](https://pokeapi.co/docs/v2#fairuse), and to use it properly with TypeScript I need types for the various calls it is capable of.  Both of these concerns are addressed by [`pokenode-ts`](https://github.com/Gabb-c/pokenode-ts), one of the [Wrapper Libraries recommended by PokéAPI](https://pokeapi.co/docs/v2#wrap) itself.  If I were developing a larger application and had other endpoints to worry about, I would likely have only used the types from this package, and gone with something more universal for fetching, caching, etc, so as to have one solution for all, rather than disparates.
