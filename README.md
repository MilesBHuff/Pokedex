...
Inside of `src`, I have the application's entrypoints and other core files, in addition to variety of themed folders.  These folders are named appropriately, and I believe no further explanation should be needed as to their purpose.  That said, I do have some extra notes to share:  I avoided using a `components` directory, since `components` is so broad as to encompass most of the app;  I chose `routes` oves `pages` and `views`, since it seemed more appropriate for how I'm using its contents;  I chose to cordone off all of the Redux code in a `redux` folder;  the core layout elements, I placed into a `layout` folder;  and miscellaneous re-useable helper functions I placed into `utility`.  
